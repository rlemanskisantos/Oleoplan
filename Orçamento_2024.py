...
if st.session_state["authentication_status"]:
    ...
    with col2:
        st.caption(f'Bem-vindo *{st.session_state["name"]}*')
    ...
    with col1:
        opcao_data = st.radio("Selecione o período desejado: ",
                            options=["Mês Atual", "Acumulado Anual", "Acumulado até o Mês Anterior", "Outro Período"])
    ...
    if opcao_data == "Mês Atual":
    ...
    elif opcao_data == "Acumulado até o Mês Anterior":
        ...
        with col2:
            st.write('📆 Seleção entre {} e {}:'.format(filtro_data_inicio, filtro_data_fim))
            st.write('📝 Favor justifique seus gastos na aba "Histórico" abaixo 👇')
    ...
    elif st.session_state["username"] == "rodrigolemanski":
        responsavel_filtro = st.selectbox('Selecione o Responsável:', ["Rodrigo Lemanski", "Francielly Costa", "Roberta Moretti","Elsa Brandalise"])

    elif st.session_state["username"] == "vivianevelloso":
        responsavel_filtro = st.selectbox('Selecione o Responsável:', ["Viviane Velloso", "Ana Lagoas", "Andreia Freitas" , "Benedito da Rocha", "Daniela Metz", "Fabiola Dutra", "Jocilene Andrade", "Ziama Mota", "Jaira Araujo"])
    
    else:
        responsavel_filtro = st.session_state["name"]
        ...
        with st.expander(f"Centro de Custo: {centro_custo_filtro}"):
            ...
            with tab3:
                ...
                for natureza_filtro in naturezas_ordenadas:
                    ...
                    df_CC_nat['Data'] = df_CC_nat['Data'].apply(lambda x: x.strftime("%d/%m/%Y")).tolist()
                    colunas_selecionadas_hist = ['Data', 'Valor Realizado', 'Fornecedor', 'Observação']
                    ...
                    if opcao_data == "Acumulado até o Mês Anterior":
                        

                        if abs(delta_nat) < 1000:
...
selected_functionality = st.sidebar.selectbox("Selecione a função:",
                                      ["Redefinir Senha", "Registro de Usuário"],
                                      index=None,
                                      placeholder="Selecione...")



###4. Creating a reset password widget to a logged in user


if st.session_state["authentication_status"] and selected_functionality == "Redefinir Senha":
    try:
...
if selected_functionality == "Registro de Usuário" and st.session_state["username"] == "admin" and st.session_state["authentication_status"]:
